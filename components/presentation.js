...
export default function Presentation() {
    ...
    function ScrollAnim(){
        ...
        gsap.to("#ScrollContainer", {duration : 4, scrollTo : {y : "3700", autoKill : false}})   
        gsap.to("#ScrollContainer", {delay: 4, overflowY:"scroll"}) 
    }
    ...
    useEffect(() => {
        ...
        var ts;
        document.getElementById("PresentationTrigger").addEventListener('touchstart', function (e){
            e.preventDefault();
            ts = e.touches[0].clientY;
        });

        document.getElementById("PresentationTrigger").addEventListener('touchend', function (e){
            var te = e.changedTouches[0].clientY;
            if(ts > te+5){
                ScrollAnim();
            }else if(ts < te-5){
                gsap.to("#ScrollContainer", {duration : 5, scrollTo : {y : "0", autoKill : false}}) 
            }
        });
        ...
        document.getElementById("PresentationTrigger").addEventListener('wheel', function(event) {
            if (event.deltaY > 0) {
                ScrollAnim();
            }
            if (event.deltaY < 0){
